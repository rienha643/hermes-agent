...
def test_slack_media_upload_with_home_target_threads_when_inbound_thread_exists(tmp_path):
    config, slack_cfg = _make_slack_config(chat_id="C0B5W21GF8A")
    image = tmp_path / "smoke_minimal_thread_00001_.png"
            ...
            send_message_tool(
                {
                    "action": "send",
                    "target": "slack",
                    "message": "comfyui result upload",
                    "artifact_files": [str(image)],
                }
            )
        )
...
def test_slack_media_upload_without_thread_context_is_blocked_and_warned(tmp_path):
    config, _slack_cfg = _make_slack_config(chat_id="C0B5W21GF8A")
    image = tmp_path / "smoke_minimal_thread_00001_.png"
    image.write_bytes(b"png")
    ...
    assert result["warn"] == "No inbound Slack context available for media delivery."
    send_mock.assert_not_awaited()
